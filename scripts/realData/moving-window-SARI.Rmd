...
for(t in 1:5) {
  ...
  for(i in seq_len(T_length)) {
    ...
    d_max <- which.min(T_length - i , D)      #  i = T_length, d_max = 0; when i = 1, d_max = T_length - 1
    x_actual <- 0:d_max        # 
  ...
  qtd_curves <- ggplot() +
    ...
    facet_wrap(
      ...
      labeller = labeller(time = function(x) {
        # ，scoreRange_SARI[t] is the latest date，
        # time_lengths[t] indicates the number of time window
        # the earliest date should be: scoreRange_SARI[t] - (time_lengths[t]-1)*7 天
    ...
    theme(
      ...
      axis.title.y = element_text(face = "bold", hjust = 0.5, margin = margin(r = 20)),
...
final_plot <- wrap_plots(plots, ncol = 6, nrow = 4) +  
  plot_annotation(
    ...
    theme = theme(
      plot.title = element_text(hjust = 0.5, size = 16, face = "bold"),  
      plot.caption = element_text(hjust = 0.5, size = 10),
      plot.margin = margin(10, 10, 50, 50)  # Leave more margin to the left
    )
  ) &
  theme(
    axis.title.x = element_text(size = 16, face = "bold")
