...
# save(out_husO104, file = file.path(path_proj, "data", "fitted_model","husO104_110525_to_110607_five_e.RData"))
out <- load(file.path(path_proj, "data", "fitted_model","husO104_110525_to_110607_five_e.RData"))
...
for(t in 1:5) {
  ...
  method_colors <- c(
    ...
    "b-Constant"     = "#b35806",  # orange
    "b-Random walk"  = "#8446c6",  # purple
    "b-OU process"   = "#4682B4",  # steel blue
    "Actual curve"   = "gray30"     # actual data in black
  )
  ...
  qtd_curves <- ggplot() +
    ...
    geom_line(data = subset(df_long, method == "Actual curve"),
              aes(x = d, y = q, color = method), size = 1) +
    geom_point(data = subset(df_long, method == "Actual curve"),
               aes(x = d, y = q, color = method), size = 1.5) +
    # Other curves
    geom_line(data = subset(df_long, method != "Actual curve"),
              aes(x = d, y = q, color = method), size = 1) +
    facet_wrap(
    ...
    theme(
      ...
      strip.text = element_text(face = "bold", size = 14),         # Smaller facet label size
      axis.title = element_text(size = 14),                          # Smaller axis title
      axis.title.y = element_text(face = "bold", hjust = 0.5, margin = margin(r = 10)),
