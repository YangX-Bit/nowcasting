...
# plot to check the report
```{r}
# 1. specify your date‐range:
start_date <- as.Date("2011-05-07")
end_date   <- as.Date("2011-06-07")

# 2. filter and pivot to long:
plot_df <- husO104 %>%
  ...
  mutate(
    delay = as.integer(sub("^delay", "", delay))
  )

plot_df <- husO104 %>%
  filter(date >= start_date, date <= end_date) %>%
  pivot_longer(
    cols      = starts_with("delay"),
    names_to  = "delay",
    values_to = "cum_count"
  ) %>%
  mutate(delay = as.integer(sub("^delay", "", delay))) %>%
  group_by(date) %>%
  mutate(
    total = cum_count[delay == max(delay)],
    prop  = ifelse(total == 0,       # if denominator = 0
                   0,                # set it to be 0
                   cum_count / total #
    )
  ) %>%
...
# 3. plot:
p_report <- ggplot(plot_df, aes(x = delay, y = prop, color = date, group = date)) +
  geom_line(size = 1) +
