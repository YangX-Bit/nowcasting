...
out <- load(file.path(path_proj, "data", "fitted_model","husO104_110527_to_110628_five.RData"))
...
```{r}
D <- 15
date_start <- as.Date("2011-05-07")

#date_end <- as.Date("2011-06-28")
date_end <- as.Date("2011-06-05")
...
df <- df %>%
  mutate(
    ...
    available_delay = as.numeric(date_end - ReportDate) + 1
  ) %>%
...
final_plot <- wrap_plots(plots, ncol = 6, nrow = 5) +  
  plot_annotation(
    title = NULL,
    theme = theme(
