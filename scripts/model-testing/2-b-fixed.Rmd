...
```{r}
fixed_b <- file.path(path_proj, "source", "models", "b-fixed.stan")
model <- cmdstan_model(fixed_b)
```
...
plot(fixed_b_FR$qd, col = "red", pch = 19, type = "b", ylim = c(0,1))
```
...
```{r}
ind = 40
stan_data <- list(T = ind, D = 15, Y = fixed_b_FR$case_reported_cumulated[1:ind, ])
test = model$sample(
    data = stan_data,
    iter_sampling = 3000,
    iter_warmup = 1000,
...
param_true = tibble(
    ...
    x = fixed_b_FR$case_true[1:ind, 1]
)
...
plot(fixed_b_NFR$qd, col = "red", pch = 19, type = "b", ylim = c(0,1))
```
...
```{r}
ind = 40
stan_data <- list(T = ind, D = 15, Y = fixed_b_NFR$case_reported_cumulated[1:ind, ])
test = model$sample(
    data = stan_data,
    iter_sampling = 3000,
    iter_warmup = 1000,
...
param_true = tibble(
    parameter = grep("^N\\[.+\\]$", varnames, value = TRUE),
    x = fixed_b_NFR$case_true[1:ind, 1]
)
mcmc_areas(test$draws("N"), prob_outer = 0.95) +
    geom_point(aes(x = x), param_true, color = "red", size = 1)
```
