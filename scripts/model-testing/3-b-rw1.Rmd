...
```{r}
rw_b <- file.path(path_proj, "source", "models", "b-rw1.stan")
model <- cmdstan_model(rw_b)
```
...
params_rw_b_FR <- list(
  ...
  q_model = list(
    method        = "rw_b",
    method_params = list(b_intercept = log(0.7), b_sigma = 0.1, phi_intercept = 1, phi_sigma = 0.1)
  )
)

rw_b_FR <- simulateData(params_rw_b_FR)
par(mfrow = c(2, 1))
plot(rw_b_FR$b, pch = 19, type = "b")
plot(rw_b_FR$phi, pch = 19, type = "b")

par(mfrow = c(1, 1))
matplot(t(rw_b_FR$qd), type = "l", lty = 1, ylim = c(0, 1))
```
...
```{r}
ind = 40
stan_data <- list(T = ind, D = 15, Y = rw_b_FR$case_reported_cumulated[1:ind, ])
test = model$sample(
    data = stan_data,
    iter_sampling = 3000,
    iter_warmup = 2000,
...
params_rw_b_NFR <- list(
  ...
  q_model = list(
    method        = "rw_b",
    method_params = list(b_intercept = log(0.2), b_sigma = 0.1, phi_intercept = 1.5, phi_sigma = 0.1)
  )
)

rw_b_NFR <- simulateData(params_rw_b_NFR)
par(mfrow = c(2, 1))
plot(rw_b_NFR$b, pch = 19, type = "b")
plot(rw_b_NFR$phi, pch = 19, type = "b")

par(mfrow = c(1, 1))
matplot(t(rw_b_NFR$qd), type = "l", lty = 1, ylim = c(0, 1))
```
...
```{r}
ind = 40
stan_data <- list(T = ind, D = 15, Y = rw_b_NFR$case_reported_cumulated[1:ind, ])
test = model$sample(
    data = stan_data,
    iter_sampling = 3000,
    iter_warmup = 2000,
