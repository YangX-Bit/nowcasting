...
# model
method = "ou_b"; 
b_alpha=0.4; b_mu=0.5; b_init=0.5; 
...
FR_1$case_true - FR_1$case_reported_cumulated[,16]
```
...
# generate
FR_2 <- simulateData(params_1)

FR_2$case_true - FR_2$case_reported_cumulated[,16]
```
...
# generate
FR_3 <- simulateData(params_1)

FR_3$case_true - FR_3$case_reported_cumulated[,16]
```


```{r}
# exploritary analysis
page_num <- ceiling(nrow(FR_1$case_reported_cumulated)/16)
exp_plot_1 <- fit_exp_plot(FR_1$case_reported_cumulated,ncol = 4, nrow = 4, page = c(1:page_num), if_fit = T)
print(exp_plot_1)


exp_b_data_1<- data.frame( date = as.Date(rownames(FR_1$case_reported_cumulated)),
...
```{r}
# exploritary analysis
exp_plot_2 <- fit_exp_plot(FR_2$case_reported_cumulated,ncol = 4, nrow = 4, page = c(1:page_num), if_fit = T)
print(exp_plot_2)


exp_b_data_2<- data.frame( date = as.Date(rownames(FR_2$case_reported_cumulated)),
...
```{r}
# exploritary analysis
exp_plot_3 <- fit_exp_plot(FR_3$case_reported_cumulated,ncol = 4, nrow = 4, page = c(1:page_num), if_fit = T)
print(exp_plot_3)


exp_b_data_3<- data.frame( date = as.Date(rownames(FR_3$case_reported_cumulated)),
...
## fit model
```{r}
out_1_FR <- nowcasting_moving_window(FR_1$case_reported_cumulated, scoreRange = scoreRange, 
...
```{r}
out_2_FR <- nowcasting_moving_window(FR_2$case_reported_cumulated, scoreRange = scoreRange, 
...
```{r}
out_3_FR <- nowcasting_moving_window(FR_3$case_reported_cumulated, scoreRange = scoreRange, 
