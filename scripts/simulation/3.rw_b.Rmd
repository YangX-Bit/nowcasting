...
#data path
data_save_path = file.path(path_proj, "data", "fitted_model", "simulation", "3. rw_b")
```
...
params_rw_b_NFR <- list(
  ...
  q_model = list(
    method        = "rw_b",
    method_params = list(b_init = 0.1, b_sigma = 0.02, phi_init = 0.1, phi_sigma = 0.05)
  )
)
...
out_rw_b_FR <-
  nowcasting_moving_window(rw_b_FR$case_reported_cumulated, scoreRange = scoreRange,
                          ...
                          posterior_draws_path = file.path(posterior_draws_path, "rw_b")
                          )
...
plots_rw_b_FR <- nowcasts_plot(results_rw_b_FR, D = D, report_unit = "day", methods = models_to_use)
