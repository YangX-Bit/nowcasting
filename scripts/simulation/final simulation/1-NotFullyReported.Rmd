...
D_trunc <- 10; D_complete <- 25; if_fully_rep <- F
...
##$ fit model
```{r}
scoreRange_1 <- seq(as.Date("2024-01-15"),as.Date("2024-02-29"),by="14 day")
out_1 <- nowcasting_moving_window(data_1$case_reported, scoreRange = scoreRange_1, 
...
results_1 <- nowcasts_plot(out_1, D = D_trunc, report_unit = "day", 
                         models_to_run = c("fixed_q", "fixed_b", "b_poly", "b_spline"))
```
...
```{r}
# all cases
for(i in 1:length(scoreRange_1)){
  print(calculate_metrics(results_1$nowcasts[i][[1]]))
}
```
...
# Within D
for(i in 1:length(scoreRange_1)){
  print(calculate_metrics(data.table::last(results_1$nowcasts[i][[1]], D_trunc)))
}
```
