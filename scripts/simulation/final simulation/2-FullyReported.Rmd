...
results_2 <- nowcasts_plot(out_2, D = D_trunc, report_unit = "day", 
                         models_to_run = c("fixed_q", "fixed_b", "b_poly", "b_spline"),
...
for(i in 1:length(scoreRange_2)){
  print(calculate_metrics(results_2$nowcasts[i][[1]]))
}
```
...
# Within D
for(i in 1:length(scoreRange_2)){
  print(calculate_metrics(data.table::last(results_2$nowcasts[i][[1]], D_trunc)))
}
```
