...
#data path
data_save_path = file.path(path_proj, "data", "fitted_model", "simulation")
```

```{r}
#models
fixed_q <- file.path(path_proj, "source", "models", "withInterceptFixed",
...
linear_b <- file.path(path_proj, "source", "models", 
                     "4.stan_model_ou_b_trunc.stan")
ou_b <-  file.path(path_proj, "source", "models", "withInterceptFixed",
                      "4.stan_model_ou_b_trunc.stan")
...
# mean(exp_plot_b_FR$coefficients$phi)

# exp_plot_fix_b_FR <- fit_exp_plot(fixed_b_FR$case_reported_cumulated, ncol = 4, nrow = 4, page = c(1:page_num), if_fit = T)


# exp_plot_b_FR <- ggplot(fixed_b_FR, aes(x = as.Date(rownames(fixed_b_FR$case_reported_cumulated)), 
#                                      y = exp_plot_b_FR$coefficients$b)) +
...
out_fixed_q_FR <- nowcasting_moving_window(fixed_q_FR$case_reported_cumulated, scoreRange = scoreRange,
                          ...
                          D = D, seeds = seed,
                          models_to_run =c("fixed_q", "linear_b", "ou_b"),
...
save(out_fixed_q_FR, file = file.path(data_save_path, "FR_fixed_q_fixedInter.RData"))
load( file.path(data_save_path, "FR_fixed_q_fixedInter.RData"))
...
for (i in 1:length(results_fixed_q_FR)) {
  print(calculate_metrics(results_fixed_q_FR[[i]],
                          methods = c("fixed_q", "linear_b", "ou_b")))
}
...
save(out_fixed_b_FR, file = file.path(data_save_path, "FR_fixed_b_fixedInter.RData"))
load( file.path(data_save_path, "FR_fixed_b_fixedInter.RData"))

results_fixed_b_FR <- nowcasts_table(out_fixed_b_FR, D = D, report_unit = "day", 
                          models_to_run = c("fixed_q", "linear_b", "ou_b"))

plots_fixed_b_FR <- nowcasts_plot(results_fixed_b_FR, D = D, report_unit = "day", models_to_run = c("fixed_q",   "linear_b" , "ou_b"))
...
```{r}
out_linear_b_FR <-
  nowcasting_moving_window(linear_b_FR$case_reported_cumulated, scoreRange = scoreRange,
...
#save(out_linear_b_FR, file = file.path(data_save_path, "FR_rw_b_fixedInter.RData"))
#load( file.path(data_save_path, "FR_rw_b_fixedInter.RData"))

results_linear_b_FR <- nowcasts_table(out_linear_b_FR, D = D, report_unit = "day", 
                          models_to_run = c("fixed_q", "linear_b", "ou_b"))

plots_linear_b_FR <- nowcasts_plot(results_linear_b_FR, D = D, report_unit = "day", models_to_run = c("fixed_q",   "linear_b" , "ou_b"))
...
for (i in 1:length(results_linear_b_FR)) {
  print(calculate_metrics(data.table::last(results_linear_b_FR[[i]],D),
                          methods = c("fixed_q", "linear_b", "ou_b")))
}
...
#save(out_ou_b_FR, file = file.path(data_save_path, "FR_ou_b_fixedInter.RData"))
#load(file.path(data_save_path, "FR_ou_b_fixedInter.RData"))
