...
for (i in 1:num_sims) {
  ...
  out_b_ou_FR_multi[[i]] <- 
    nowcasting_moving_window(b_ou_FR_multi[[i]]$case_reported_cumulated, scoreRange =  as.Date("2024-01-30"),
...
#
results_b_ou_FR <- nowcasts_table(out_b_ou_FR_multi[[1]], D = D, report_unit = "day", 
                          methods = models_to_use)
...
### Metrics table
```{r}
highlight_metrics(metrics_1_b_ou_FR_avg, method_names = c("Constant q", "Constant b", "Randow Walk b","OU b"), date_labels = scoreRange, table_caption = "Fully Reported + OU b (All Time Window)")
